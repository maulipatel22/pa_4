...
public class MyDBFaultTolerantServerZK extends MyDBSingleServer implements Watcher {
    ...
    private void loadCheckpoint() {
        try {
            ...
            if (row != null) {
                ...
                log.info("No existing checkpoint for " + myID
                        + "; starting from scratch");
            }
        } catch (Exception e) {
            log.log(Level.SEVERE,
                    "Error reading checkpoint for " + myID, e);
        }
    }
    ...
    private void persistCheckpoint() {
        try {
            ...
            log.log(Level.SEVERE,
                    "Error persisting checkpoint for " + myID, e);
        }
    }

    private void applyRequestToDB(String cql) {
        try {
            session.execute(cql);
        } catch (Exception e) {
            log.log(Level.SEVERE, "Error applying CQL to DB: " + cql, e);
        }
    }
    ...
    private synchronized void replayPendingRequests() {
        try {
            ...
            for (String child : children) {
                if (lastAppliedZnode != null &&
                        child.compareTo(lastAppliedZnode) <= 0) {
                    continue;
                }
            ...
            log.log(Level.INFO,
                    "No " + REQUESTS_PATH + " znode yet; nothing to replay");
        } catch (Exception e) {
    ...
    protected void handleMessageFromClient(byte[] bytes, NIOHeader header) {
        String cql = new String(bytes, StandardCharsets.UTF_8).trim();
        if (cql.isEmpty()) {
            return;
        }
        try {
            ...
            try {
                clientMessenger.send(header.sndr,
                        "OK".getBytes(StandardCharsets.UTF_8));
            } catch (IOException e) {
                log.log(Level.SEVERE,
                        "Error sending response to client", e);
            }
        } catch (KeeperException | InterruptedException e) {
            log.log(Level.SEVERE,
                    "Error logging client request in ZooKeeper", e);
        } catch (Exception e) {
            log.log(Level.SEVERE,
                    "Unexpected error handling client request", e);
        }
    }
    ...
    public void close() {
        try {
            ...
            log.log(Level.WARNING,
                    "Interrupted while closing ZooKeeper", e);
        }
        try {
            if (session != null) session.close();
        } catch (Exception e) {
            log.log(Level.WARNING,
                    "Error closing Cassandra session", e);
        }
        try {
            if (cluster != null) cluster.close();
        } catch (Exception e) {
            log.log(Level.WARNING,
                    "Error closing Cassandra cluster", e);
        }
        super.close();
    }
